...
def dual(x, arcs, Voxels, A, D, Beamlets, Struct_list, S_dict, max_constrs = {"dummy" : 1000000}, min_constrs = {"dummy": -1000000}):
    ...
    delta = model.addVars(arcs, vtype=GRB.CONTINUOUS, name="delta", lb=0)
    ...
    if model.status == GRB.OPTIMAL:
        ...
        ray_delta = {k: delta[k].UnbdRay for k in delta}

        return ray_pi_plus, ray_pi_minus, ray_mu_u, ray_mu_l, ray_delta, None, "unbounded"

    else:
        raise Exception("dual problem not solved correctly")
def build_feasibility_cut(ray_pi_plus, ray_pi_minus, ray_mu_u, ray_mu_l, ray_delta, x_vals, D, max_constrs, min_constrs):
    ...
    for i in range(len(x_vals)):
        ...
        # Contributions from delta variables
        for i, j in arcs:
            coeff += ray_delta[i, j] * (x_vals[i] + x_vals[j] - 1)  # Contribution of delta in the cut
...
def master(cuts, feas_cuts, nodes, arcs, voxels, D, pi_plus, pi_minus, mu_u, mu_l, delta, max_constrs, min_constrs, optimality_cut): #= {"dummy" : 1000000}, min_constrs = {"dummy": -1000000}):
    ...
    if optimality_cut:
        ...
        for i, j in arcs:
            val += (x[i] + x[j] - 1) * delta[i, j]  # Add delta to optimality cut
...
pi_plus, pi_minus, mu_u, mu_l, delta, dual_obj, status = dual(x=mis,arcs = arcs, Voxels=A.shape[0], A=A, D=D, Beamlets=A.shape[1], Struct_list=roi_list, S_dict=voxel_dict, max_constrs=max_constrs, min_constrs=min_constrs)
optimality_cut = True
...
while gap != TOL and iteration <= MAX_ITERS:
    ...
    pi_plus, pi_minus, mu_u, mu_l, delta, dual_obj, status = dual(
        x=x_vals, arcs=arcs, Voxels=A.shape[0], A=A, D=D, Beamlets=A.shape[1],
    ...
    if status == "unbounded":
        ...
        dual_obj = dual_obj - l*sum(x_vals)
