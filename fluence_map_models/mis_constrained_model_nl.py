...
def optimization(A, D, nodes, arcs, beamlets = 8682, voxels = 212367, mis_size = 3, mis = [], roi_list = [], voxel_list = [], max_constrs = {}, avg_constrs = {}, dvh_constrs = {}, prescription = {}, weights = {}):
    #beamlets = 10
    options = {
        "WLSACCESSID" : "55c88bbb-cfc0-4d09-b763-e37e0d3bd302",
        "WLSSECRET" : "f21e8941-0ff2-49c2-8500-6883437c1bb2",
    ...
    pres = [20 for i in range(voxels)]

    for struct, val in prescription.items():
        if struct in roi_list:
    ...
    for i in range(len(pres)):
        if np.sum(A[i]) == 0:
            pres[i] = 0
            #print(f"Alert: voxel {i} is not targeted by any beamlet.")
            count += 1
    '''

    #print(f"{count} of {len(pres)} voxels not targeted by any beamlet.")

    roi_to_voxels = dict(zip(roi_list, voxel_list))

    w = [1 for i in range(voxels)]
    '''
    for struct, val in weights.items():
    ...
    #model.setObjective(sum((A@y - (peak_weight*D@x + pres)) * (A@y - (peak_weight*D@x + pres))))


    model.addConstr(z >= w*(A@y - (peak_weight*D@x + pres)))
    model.addConstr(z >= -1*w*(A @ y - (peak_weight * D @ x + pres)))
    ...
    print("max constr loaded")

    for struct, av in avg_constrs.items():
        if struct in roi_list:
            u_vox = voxel_list[roi_list.index(struct)][-1]
            l_vox = voxel_list[roi_list.index(struct)][0]
            model.addConstr(sum((A@y)[l_vox:u_vox])/(u_vox-l_vox) <= av)

    print("avg constr loaded")
    ...
    print("dvh constrs loaded")

    rhs = np.full(voxels, 1000)

    #model.addConstr(A @ y <= rhs)

    model.Params.TimeLimit = 20100
    ...
    deviation = 0
        ...
        #
    # Collect solved values of z[i] into a list

    y_sol = y.X
    dose = A@y_sol

    beamlet_use = list(y_sol)

    x_sol = x.X

    node_use = list(x.X)

    deviation = (A @ y_sol - (peak_weight * D @ x_sol + pres))

    # Report per-structure statistics
    # Create mapping once if not already available
    roi_to_voxels = dict(zip(roi_list, voxel_list))
    ...
    with open("mis_constr_node_results.csv", 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(node_use)

    print(f"total deviation: {sum(deviation)}")
    print(f"for mis size {sum(node_use)}")
...
prescription ={"max":0, "avg":0, "dvh":0}
weights = {roi : 1 for roi in roi_list}
weights["PTV"] = 3

#mis_sets = find_mis_sets(presolve(nodes,arcs, 60), num_runs=8)
mis_size, mis = presolve(nodes,arcs, 60)
results = []
optimization(A, D, nodes, arcs, voxels=A.shape[0], beamlets=A.shape[1], mis_size=mis_size, mis = mis, roi_list=roi_list, voxel_list=voxel_list, max_constrs=max_constrs, avg_constrs=avg_constrs, dvh_constrs=dvh_constrs, prescription = prescription, weights = weights)
