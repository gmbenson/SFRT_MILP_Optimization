...
def optimization(A, D, nodes, arcs, beamlets = 8682, voxels = 212367, mis_size = 3, mis = [], roi_list = [], voxel_list = [], max_constrs = {}, avg_constrs = {}, dvh_constrs = {}, prescription = {}, weights = {}):
    ...
    deviation = [float(z[i].X) for i in range(voxels)]
...
weights = {roi : 1 for roi in structure_list}
weights["PTV"] = 3
...
results = []
optimization(A,D,nodes,arcs, voxels=A.shape[0], beamlets=A.shape[1], mis_size=mis_size, mis = mis, roi_list=structure_list, voxel_list=voxel_list, max_constrs=max_constrs, avg_constrs=avg_constrs, dvh_constrs=dvh_constrs, prescription = prescription, weights=weights)
